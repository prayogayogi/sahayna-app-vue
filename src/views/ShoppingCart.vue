                                      ...
                                      <tbody v-if="keranjangUser.length > 0">
                                          <tr v-for="keranjang in keranjangUser" :key="keranjang.id">
                                              ...
                                              <td class="delete-item">
                                                    <a @click="removeItem(keranjangUser.id)" href="#"><i class="material-icons"> close</i></a>
                                              </td>
                                          </tr>
                                      </tbody>
                                      <tbody v-else>
                                          <tr id="shop">
                                              <p>Product Tidak Ada Yang Dimasukan Shopping Chart</p>
                                          </tr>
                                      </tbody>
                                  </table>
                              </div>
                          ...
                          <div class="col-lg-12 text-left">
                              <div class="proceed-checkout">
                                 ...
                                 <!-- <router-link to="/success"> -->
                                  <a href="#" @click.prevent="checkOut()" class="proceed-btn">I ALREADY PAID</a>
                                  <!-- </router-link> -->
                              </div>
                          </div>
                      </div>
...
export default {
  ...
  checkOut(){
     ...
     let ekskusi = axios.post("http://127.0.0.1:8000/api/checkout",checkoutData)
      .then(() => this.$router.push("success"))
      .catch(err => console.log(err));

        if(ekskusi){
            this.keranjangUser.splice(0)
            const parsed = JSON.stringify(this.keranjangUser);
            localStorage.setItem('keranjangUser', parsed);
        }
